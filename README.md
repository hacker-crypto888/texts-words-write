...
| Cancel the edits made to the texts using the editor and save all texts | <ul><li>[ ] </li></ul> | <ul><li>[x] </li></ul> | Each connected user can cancel its edits and save all texts including that from other users | I added a line of code to save the edits each time an item is removed. I added the variables containing the lists at their initial state. I added an "onclick" event on the modal allowing to edit entries and to remove items. On click of the button, the value of the variable parsed in the beginning of the function takes the initial value of the list when the items are loaded.  
