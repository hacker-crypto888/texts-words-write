...
| Cancel the edits made to the texts using the editor and save all texts | <ul><li>[ ] </li></ul> | <ul><li>[x] </li></ul> | Each connected user can cancel its edits and save all texts including that from other users | First off, I resorted texts in the upload function into "TEXTS FROM CURRENT SESSION, TEXT FROM PREVIOUS SESSION". I added the variables containing the lists at their initial state. I added an "onclick" event on the modal allowing to edit entries and to remove items, and then to cancel the modifications. On click of the button, the value of the constant variable takes the initial value of the list when the items are loaded. The name of the values for the connected user are: <br/> importedTexts.dataset... <br/> texts => current session <br/> prev => previoous sessions of logged in user <br/> allusers => all users' sessions <br/> 
